...
const createBookingRequest = async (req, res) => {
  const { facility, intendedActivity, date, startTime, endTime } = req.body;
  const facilityId = facility; // Support both or use the one from frontend
  ...
  try {
    ...
    // 1. Strict Time Slot Validation
    const bookingDate = new Date(date);
    const dayOfWeek = bookingDate.toLocaleDateString('en-US', { weekday: 'long' });
    
    const validSlot = facility.availableSlots.find(slot => 
      slot.day === dayOfWeek && 
    ...
    if (!validSlot) {
      return res.status(400).json({ 
        message: `Booking must be within available slots for ${dayOfWeek} (${facility.availableSlots.filter(s => s.day === dayOfWeek).map(s => `${s.startTime}-${s.endTime}`).join(', ') || 'No slots available'})` 
      });
    }

    // 2. Improved Overlapping Check (Capacity based)
    // Find all approved bookings that OVERLAP with the requested time
    ...
    if (overlappingBookings.length >= facility.capacityLimit) {
      return res.status(400).json({ message: 'Facility capacity reached for this time period' });
    }
...
const getBookingRequests = async (req, res) => {
  try {
    ...
    if (isAdmin) {
      // Admin sees ALL pending requests across the system
      query = { status: 'pending' };
    } else {
      // Staff only sees pending for assigned facilities
      query = { 
        facility: { $in: req.user.assignedFacilities || [] }, 
        status: 'pending' 
      };
    }
